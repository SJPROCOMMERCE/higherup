...
export async function POST(req: Request) {
  ...
  const db = genxDb()
  const { data: lg } = await db.from('lead_generators').select('referral_code').eq('id', session.lgId).single()
  ...
  const { data: existing } = await db.from('referral_links').select('id').eq('link_code', linkCode).single()
  if (existing) return Response.json({ error: 'Link for this source already exists' }, { status: 409 })

  const { data, error } = await db.from('referral_links').insert({
