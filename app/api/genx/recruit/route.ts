...
export async function GET() {
  ...
  const [lgRes, clicksRes, referralsRes] = await Promise.all([
    db.from('lead_generators').select('referral_code, total_vas').eq('id', lgId).single(),
    db.from('referral_clicks').select('id', { count: 'exact' }).eq('lg_id', lgId),
    db.from('referral_tracking').select('va_user_id, status, referred_at').eq('lg_id', lgId),
  ])
  ...
  const [clicksLast7Res, signupsLast7Res] = await Promise.all([
    db.from('referral_clicks').select('id', { count: 'exact', head: true }).eq('lg_id', lgId).gte('clicked_at', sevenDaysAgo),
    db.from('referral_tracking').select('id', { count: 'exact', head: true }).eq('lg_id', lgId).gte('referred_at', sevenDaysAgo),
  ])
